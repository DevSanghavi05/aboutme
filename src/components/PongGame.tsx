...
export function PongGame({ openByDefault = false }: { openByDefault?: boolean }) {
  ...
  const drawInvaderStore = useCallback(() => {
    ...
    return (
      <div className="pointer-events-auto w-[min(760px,94vw)] rounded-3xl border border-zinc-700 bg-zinc-950/95 shadow-2xl flex flex-col max-h-[94vh] overflow-hidden">
        ...
        <div className="overflow-y-scroll overscroll-contain touch-pan-y flex-1 min-h-0 px-6 md:px-8 pb-2" style={{ scrollbarWidth: 'auto', scrollbarColor: '#52525b transparent', WebkitOverflowScrolling: 'touch' }}>
  ...
  }, [bankCoins, bulletType, purchaseUpgrade, upgrades, toggleFullscreen, isFullscreen]);
  ...
  useEffect(() => {
    ...
    if (arcadeGame === 'invaders') {
      ...
      const onKeyDown = (e: KeyboardEvent) => {
        ...
        if (key === 'f') { void toggleFullscreen(); e.preventDefault(); }
        if (key === 'q') {
      ...
      const drawInvaderSprite = (x: number, y: number, width: number, height: number, tint: string, frame: 0 | 1) => {
        const sprite = INVADER_SPRITE_FRAMES[frame];
      ...
      const drawEnemy = (enemy: Enemy, now: number) => {
        const { x, y, width: w, height: h, hp, maxHp } = enemy;
        const hpFrac = hp / maxHp;
        const tint = hpFrac > 0.65 ? '#7dff9a' : hpFrac > 0.35 ? '#ffd166' : '#ff6b6b';
        const frame: 0 | 1 = Math.floor(now / 280) % 2 === 0 ? 0 : 1;

        ctx.save();
        ctx.shadowColor = tint;
        ctx.shadowBlur = 8;
        drawInvaderSprite(x, y, w, h, tint, frame);
        ctx.restore();
      };
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === 'f' || e.key === 'F')  { void toggleFullscreen(); e.preventDefault(); }
    };
  ...
  }, [isOpen, gameState, difficulty, mode, arcadeGame, spawnInvaderWave, finishInvaders, finishPong, toggleFullscreen]);
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-zinc-950/90 backdrop-blur-md flex items-center justify-center">
      ...
      {/* Store overlay (single source of truth) */}
      {arcadeGame === 'invaders' && storeOpen && (
        <div className="absolute inset-0 z-30 flex items-center justify-center p-4 bg-black/45" onClick={() => setStoreOpen(false)}>
